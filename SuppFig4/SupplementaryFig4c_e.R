#load data
design<- read.table("design_asfield.txt", header=TRUE, sep="\t")
#bacteria
...
d<- design[design$Fraction %in% c("Soil"),]
d<- d[d$Genotype %in% c("Italian", "Swedish"),]
...
#merge coordinates with design data
...
adonis(bray_curtis ~ Soil *Chamber * Genotype,      data = d, permutations = 999, method = "bray", strata = NULL) 
...
var<- variance[variance$Experiment %in% c("Chamber"), ]
