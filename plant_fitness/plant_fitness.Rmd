...
sterilised$pve <- data.frame(
  ...
  mutate(Term = fct_relevel(
    Term,
    "Climate", "geno", "Soil", "Sterile", "Climate:geno", "Soil:Sterile", "Climate:geno:Sterile"
  )) %>% 
  arrange(Term) %>% 
  ggplot(aes(x = Term, y= Variance*100)) +
  geom_col() +
  ...
  scale_x_discrete(
    labels = c('Chamber', "Genotype", "Soil", "Sterilisation", 'Chamber x\nGenotype',  "Sterilisation\nx Soil", "Chamber x\nGeno. x Soil", "Other terms")
  ) +
  labs(
...
sterilised$interaction <- gnoto %>% 
  ...
  mutate(
    Soil = ifelse(Soil == "Italy", "Italian soil", "Swedish soil"),
    Genotype = ifelse(geno == "It", "Italian", "Swedish"),
    Sterilised = ifelse( Sterile == 1, "Yes", "No")
  ) %>% 
  ggplot(aes(
    x = Climate, y = mean, colour = Genotype,
    shape = Sterilised, group= paste(Genotype, Sterilised),
  )) +
  geom_point(size = 3) + 
  geom_line() +
  facet_grid(~ Soil) +
...
Figure \@ref(fig:native-sterile-repopulated) shows the increase in fitness on sterilised soils followed by the drop when you repopulate them.

(ref:nsr) Mean plant fitness in unsterilised native soils, sterilised soils, and soils sterilised and reinoculated with its native inoculum.

```{r native-sterile-repopulated, fig.cap="(ref:nsr)", fig.width=8/2.54, fig.height=11/2.54}
gnoto %>% 
